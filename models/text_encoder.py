...
class BertLayer(nn.Module):
    def __init__(self, config, layer_num, as_decoder=False):
        super().__init__()
        self.config = config
...
class BertEncoder(nn.Module):
    def __init__(self, config, as_decoder=False):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertLayer(config,i, as_decoder) for i in range(config.num_hidden_layers)])
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True, as_decoder=False):
        super().__init__(config)
        self.config = config

        self.embeddings = BertEmbeddings(config)
        
        self.encoder = BertEncoder(config, as_decoder=as_decoder)
